...
floatAdd:			#浮点数加法
	...
	addi	$t1,$0,8	
	lw	$t2,num1($t1)	#t2存num1的小数部分
	lw	$t3,num2($t1)	#t3存num2的小数部分
	add	$t4, $t2,$t3	# $t4 =$t2+ $t3 小数部分相加 t4寄存小数部分包括进位 由于3位小数可能产生4位 第4位进位
	andi	$s2，$t4,0x0007		#s2用于储存结果小数三位不包括进位
	
	addi	$t1,$0,4	
	lw	$t2,num1($t1)	#t2存num1的整数部分
	lw	$t3,num2($t1)	#t3存num2的整数部分
	add	$s3, $t2, $t3	# $s3 = $t2 + $t3 	s3存结果的整数部分
	
	srl	$t4,$t4,3	#如果小数产生进位则t4 会有进位1，如果没进位t4变为0
	add	$s3,$s3,$t4	#加上进位或不变
	
	sll	$s3,$s3,6	#左移6位 放到整数位置
	sll	$s2,$s2,3	#左移两位 放置到小数的位置上

	or	$s1,$s2,$s3	#s1用于储存最后的答案0001 110000 表示1.11
